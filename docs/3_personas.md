# Personas

## Público-alvo

O projeto é direcionado para alunos e professores do centro universitário FEI, especialmente voltado para membros que gostariam de economizar nas suas idas e voltas do campus.

### Matheus Silva, 17 anos
- Status: Primaria
- Emprego: Procurando o primeiro emprego/estágio
- Renda: mesada dos pais para coisas básicas (inclui deslocamento, alimentação, lazer)
- Objetivos: Se deslocar de maneira segura e barata para a FEI 
- Habilidades: Ensino superior incompleto, sem carta de direção, usa aplicativos a vida inteira
- Tarefas: cadastrar rotina, entrar em uma viagem, criar uma viagem, iniciar viagem como passageiro
- Requisitos: o APP vou pra FEI, Estar matriculado
- Expectativas: Conseguir economizar dinheiro no seu deslocamento para a FEI

<img width="512" height="768" alt="ChatGPT Image 23 de set  de 2025, 22_12_39" src="https://github.com/user-attachments/assets/849c73ff-a81d-4ad1-a713-5c2824b26da1" />


### Fernanda Oliveira, 21 anos
- Status: Primaria
- Emprego: Estagiária na Volkswagen
- Renda: R$1.700,00
- Objetivos: Ter com quem rachar a gasolina na ida pra FEI
- Habilidades: Ensino superior incompleto, já dirige a um tempo
- Tarefas: Cadastrar rotina, criar viagem, iniciar viagem como motorista
- Requisitos: o APP vou pra FEI, Estar matriculado
- Expectativas: Conseguir economizar na gasolina 

<img width="512" height="768" alt="d728ca2c-b6ca-4a2a-bf8e-ee6c8b23c9f4" src="https://github.com/user-attachments/assets/2848a3f7-2b1d-42d0-9a0b-92c83be0fd18" />


### Patricia Wicks, 21 anos
- Status: Primaria
- Emprego: Estágio na XP investimentos
- Renda: R$3.000,00
- Objetivos: Efetivação e formação
- Habilidades: Ensino superior incompleto, dirige bem, afinidade casual com tecnologia
- Tarefas: cadastrar rotina, entrar em uma viagem, criar uma viagem, iniciar viagem como motorista
- Requisitos: o APP vou pra FEI, Estar matriculado
- Expectativas: No máximo utiliza o app para levar amigos e conhecidos prévios para juntar as CO2ins de horas complementares

![vouprafei - persona - patricia](https://github.com/user-attachments/assets/ec64717c-bfff-4f79-8977-bfdb1b057cf5)


### Outras personas (secundárias)
- Apenas motoristas de aplicativo de fora do centro universítario FEI


# Mapa de empatia

![Mapa de empatia](imagens/empatia.png)


### **Mapa de Empatia – Persona 1: Matheus - Aluno Passageiro**

**Perfil resumido:** Matheus é aluno recem matriculado na FEI, tendo apenas 17 anos e iniciando sua trajetoria profissional. Não possui estágio nem carteira de motorista mas possui a necessidade de frequentar o campus diariamente.

**O que ele vê?**
* Preço muito volátil de motorista de aplicativos.
* Transporte público de baixa qualidade, desconfortável.

**O que ele ouve?**
* Notícias sobre insegurança de pegar apps de aplicativo desacompanhado.
* Notícias sobre o aumento de preço de combustíveis 

**O que ele pensa e sente?**
* Quer conforto e praticidade .
* Quer pagar mais barato em seu deslocamento.

**O que ele fala e faz?**
* Reclama da qualidade dos transportes públicos.
* Reclama da qualidade dos motoristas de aplicativo.

**Dores (Medos, Obstáculos)**
* Incerteza do preço da corrida.
* Incerteza da qualidade da corrida.

**Ganhos (Necessidades, Benefícios esperados)**
* Viagens mais baratas pelo custo compartilhado.
* Motorista fixo, confiável (em alguns casos).
* viagem rotineira, com menos risco de imprevisibilidade.
* Maior conforto para o deslocamento até a faculdade .

# Contexto de uso

O aplicativo será utilizado por alunos e professores do centro universitário FEI, se trata de uma aplicação movel.

# Jornada do usuário

### Usuário Passageiro
Quando matheus abrir o aplicativo pela primeira vez, ira cadastrar suas informações de rotinas de viagem (Local de ida, local de volta e quais dias da semana esse padrão se repete). Após esse cadastro, matheus irá receber sugestões de grupos ja existentes com base na sua rotina cadastrada. Matheus esta atrás de um motorista, logo a viagem da Fernanda que mora a 1 KM de sua casa é conveniente. Matheus ira solicitar para entrar no grupo de Fernanda e após aprovação basta aguardar a viagem.
Quando o dia e horario da viagem chegarem, Matheus receberá uma notificação de que Fernanda abriu viagem e então ele deve confirmar sua presença. Matheus pode acompanhar o deslocamento de Fernanda atráves de um mapa fornecido pelo aplicativo, quando Fernanda está próxima de Matheus ele já se posiciona para embarcar rapidamente. Uma vez no veículo de Fernanda, Matheus viaja até a FEI com segurança. Matheus e Fernanda agora devem viajar rotineiramente pela praticidade.

### Usuário Motorista
Fernanda também irá cadastrar sua rotina no aplicativo, porém ela esta procurando passageiros para sua viagem. Fernanda irá criar um grupo, inserindo sua rotina de viagem nele. Logo, solicitações de embarque, podendo aprovar ou recusar. Quando reunir passageiros o suficiente, e estiver no horario da viagem, Fernanda poderá abrir uma corrida, permitindo que seus passageiros confirmem presença. Fernanda então recebe o trajeto de sua viagem, com a melhor ordem de parada entre apenas os passageiros que confirmaram a corrida. Fernanda pode utilizar um provedor de mapas terceiro para seu deslocamento e deve então buscar seus colegas de viagem.




[1] Fonte: Adaptado de <https://hazeshift.com.br/mapa-de-empatia/>
